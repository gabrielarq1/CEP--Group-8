...
=======
  - "Gabriela Raygoza Quintero "
>>>>>>> 743f34f (Complete collaboration evidence and verify required links)
  - "Alondra Rodriguez"
  - "Veronica Vega"
date: today
format:
...
In addition, these tokens need to be protected from misuse. Access tokens are used to grant access to systems and services without having to re-enter login credentials such as username and password. This can be repositories, cloud services, databases and APIs. As the granted permissions of an access token it can cause a lot of harm, tokens must not be published publicly, put in screenshots or added to Git repositories. In case an access token gets lost accidentally, it must be immediately replaced with a new token.
